...
void servo_alarm(void){ OC1RS = 4000; }
void servo_reset(void){ OC1RS = 3000; }
...
void LCD_function_main(void)
{
    ...
    switch(state)
    {
        case INFO:    info_state(); break;
        case SETTEMP: settemp_state(); break;
        case SETTIME: settimer_state(); break;
        case ARM:     arm_state(); break;
        case DISSABLE:  Dissable_state(); break;
        case CONFIRM: confirm_state(); break;
...
void settemp_state(void)
{
    ...
    {
        if(JoyStickValueY > 600) adjTemp++;
        if(JoyStickValueY < 400) adjTemp--;
        lastStep = rollover;
    }
...
void settimer_state(void)
{
    ...
    if(rollover - lastStep > 120)
    {
        if(JoyStickValueY > 600) adjTimer++;
        if(JoyStickValueY < 400 && adjTimer>0) adjTimer--;
        lastStep = rollover;
    }
...
void confirm_state(void)
{
    ...
    if(JoyStickValueX < 400) sel=0;
    if(JoyStickValueX > 600) sel=1;
    ...
    if(click)
    {
        ...
        if(sel==0)
        {
            if(confirmTarget==1) maxTemp=adjTemp;
            if(confirmTarget==2) timerSet=adjTimer;
        }
...
void alarm_state(void)
{
    ...
    if(buttonState==0)
    {
        if(holdStart==0) holdStart=rollover;
